...
class MRID(blankSeq.MRIBLANKSEQ):
    def __init__(self):
        ...
        self.addParameter(key='shimming', string='Shimming (*1e4)', val=[12.0, 12.0, 5.0], field='OTH')
        ...
        self.addParameter(key='Bandwith',string='Bandwith (kHz)', val=100, field='SEQ')
        # self.addParameter(key='sweepTE', string='Sweep TE (bool)', val=0, field='OTH')
        self.addParameter(key='refocusing_phase',string='Alternating rafocusing phase?', tip='yes = 1, no = 0', val=1, field='SEQ')
    ...
    def sequenceAtributes(self):
        ...
        self.Bandwith = self.Bandwith * 1e3  # Hz
    ...
    def sequenceRun(self, plotSeq=0, demo=False):
        ...
        self.flip_angle =  np.loadtxt(r'C:\Users\User\MRID_parameters\FA(1s)100(4lobulos).txt',delimiter=',')
        ...
        self.echo_time = (np.loadtxt(r'C:\Users\User\MRID_parameters\TE(1s)100(0,1-0.9TR).txt',delimiter=','))* 1e-3  # s
        ...
        sampling_rate = 1/BW   # s

        self.mapVals['sampling_rate'] = sampling_rate
        # self.mapVals['kMax'] = kMax
        # self.mapVals['dK'] = dK
        nPPL_rad= np.zeros((self.n_repetition,1))
        nPPL_dia= np.zeros((self.n_repetition,1))

        for kk in range(self.n_repetition):
            nPPL_rad[kk] = int((self.echo_time[kk]/2-2*self.dead_time-self.rf_time) * BW )-hw.addRdPoints #(addPoints so the acquisition windows do not overlap)
            nPPL_dia[kk]= int((self.repetition_time-self.echo_time[kk]/2-self.rf_time-2*self.dead_time) * BW )-hw.addRdPoints

        print("\nExpected acquired points %i" % (
                    (np.sum(nPPL_dia) + np.sum(nPPL_rad) + 4 * hw.addRdPoints * n_repetition) * hw.oversamplingFactor))

        if np.any(nPPL_dia<=0) or np.any(nPPL_rad<=0):
            print('Too few points. Please select a greater Bandwith')
            return False

        acq_time_rad = (nPPL_rad) / BW * 1e6 # us
        acq_time_dia = (nPPL_dia) / BW * 1e6 # us
        ...
        def createSequence():
            ...
            while repeIndex < self.mapVals['n_repetition'] :
                ...
                self.rxGateSync(t0rx, acq_time_rad[repeIndex], channel=rx_channel)
                acq_points += nPPL_rad[repeIndex]
                acq_rad_points += nPPL_rad[repeIndex]

                # Refocusing pulse
                trf0 = trf0 + echo_time[repeIndex] / 2
                if self.refocusing_phase==1:
                ...
                # Rx gate Refocusing
                t0rx = trf0 + hw.blkTime + RFpulsetime + TxRxtime
                self.rxGateSync(t0rx, acq_time_dia[repeIndex], channel=rx_channel)
                acq_points += nPPL_dia[repeIndex]
                acq_dia_points += nPPL_dia[repeIndex]

                if repeIndex == self.n_repetition - 1:
                    self.endSequence(t0 + 2 * tr)

                repeIndex = repeIndex + 1
                # diaIndex= diaIndex + 1
            return acq_points, acq_rad_points, acq_dia_points

        # Time parameters to microseconds
        sampling_rate = self.mapVals['sampling_rate'] * 1e6  # us
        self.mapVals['sampling_rate'] = sampling_rate # us
        over_data_rad = []
        over_data_dia = []
        over_data = []
        data_full = []
        if not self.demo:
            ...
            self.expt = ex.Experiment(lo_freq=self.larmor_freq*1e6, rx_t=sampling_rate, init_gpa=init_gpa,
            ...
            # Create the sequence
            acq_points, acq_rad_points, acq_dia_points = createSequence()
            print('\nExpected acquired points: %i' % ((acq_points+4*hw.addRdPoints*n_repetition)*hw.oversamplingFactor))
            ...
            if plotSeq==0:
                ...
                for scan in range(nScans):
                    ...
                    for repetition in range(self.mapVals['n_repetition']):
                        ...
                        n_ini = int((pts_rad_in_rep + pts_diam_in_rep + 4 * hw.addRdPoints*repetition * hw.oversamplingFactor))
                        pts_rad_in_rep = int(pts_rad_in_rep + nPPL_rad[repetition]*hw.oversamplingFactor)
                        n_fin = int(n_ini + (nPPL_rad[repetition] + 2 * hw.addRdPoints) * hw.oversamplingFactor)
                        #over_data_rad = np.concatenate((over_data_rad, data[n_ini:n_fin]), axis=0)
                        data_dec_rad=self.decimate(data[n_ini:n_fin], 1)
                        data_full_rad = np.concatenate((data_full_rad, data_dec_rad), axis=0)

                        pts_diam_in_rep = pts_diam_in_rep + nPPL_dia[repetition]*hw.oversamplingFactor
                        # Concatenate points after refocusing
                        n_ini = n_fin * 1
                        n_fin = int(n_ini + (nPPL_dia[repetition] + 2 * hw.addRdPoints) * hw.oversamplingFactor)
                        # over_data_dia = np.concatenate((over_data_dia, data[n_ini:n_fin],1), axis=0)
                        data_dec_dia= self.decimate(data[n_ini:n_fin],1)
                        data_full_dia = np.concatenate((data_full_dia,data_dec_dia), axis=0)

                    if scan==0:
                        data_full_rad_scan = np.reshape(data_full_rad,[len(data_full_rad),1])
                        data_full_dia_scan =np.reshape(data_full_dia,[len(data_full_dia),1])
                    else:
                        data_full_rad_scan = np.concatenate((data_full_rad_scan,np.reshape(data_full_rad,[len(data_full_rad),1])),axis=1)
                        data_full_dia_scan = np.concatenate((data_full_dia_scan,np.reshape(data_full_dia,[len(data_full_dia),1])),axis=1)


                for kk in range(self.n_repetition):
                    data_full_dia[kk]= np.mean(data_full_dia_scan[kk][:])
                    data_full_rad[kk]= np.mean(data_full_rad_scan[kk][:])
                        # Decimate the results
                    # data_full_rad = self.decimate(over_data_rad, self.n_repetition*self.mapVals['nScans'])
                    # data_full_dia = self.decimate(over_data_dia, self.n_repetition*self.mapVals['nScans'])
        ...
        # Decimate the results
            ...
            for repetition in range(self.n_repetition * self.nScans):
                    # Add "after excitation" data
                n_ini = int(n_rad)
                n_rad= n_rad+nPPL_rad[repetition]
                n_fin = int(n_rad)
                data_full = np.concatenate((data_full, data_full_rad[n_ini:n_fin]), axis=0)

                    # Add "after refocusing data
                n_ini=int(n_dia)
                n_dia = n_dia+nPPL_dia[repetition]
                n_fin = int(n_dia)
                data_full = np.concatenate((data_full, data_full_dia[n_ini:n_fin]), axis=0)
    ...
    def sequenceAnalysis(self, obj=''):
        ...
        # Load data
        data_full = (self.mapVals['dataFull'])
        data_rad = (self.mapVals['dataFullrad'])
        data_dia = (self.mapVals['dataFulldia'])
        ...
        echo_time = self.mapVals['echo_time']*1e6 # ms
        repetition_time = self.mapVals['repetition_time']*1e6 # us
        ...
        for kk in range(self.mapVals['n_repetition']):
            echo_point = int(np.ceil(echo_time[kk] / (2 * sampling_period)))
            t_ini = self.rf_time / 2 + self.dead_time + repetition_time*kk
            t_fin = t_ini + (nPPL_rad[kk] - 1) * sampling_period
            time_vector_rad = np.linspace(t_ini, t_fin, int(nPPL_rad[kk]))
            t_ini = repetition_time*kk + echo_time[kk] / 2 + self.rf_time / 2 + self.dead_time + sampling_period / 2
            t_fin = t_ini + (nPPL_dia[kk] - 1) * sampling_period
            time_vector_dia = np.linspace(t_ini, t_fin, int(nPPL_dia[kk]))

            if kk==0:
                time_vector = time_vector_rad
                time_vector = np.concatenate((time_vector, time_vector_dia), axis=0)
            else:
                time_vector = np.concatenate((time_vector, np.array(time_vector_rad)), axis=0)
                time_vector = np.concatenate((time_vector, time_vector_dia), axis=0)

            echo_max=np.abs(data_dia[int(pt_echo+echo_point)])
            max_echo=np.concatenate((max_echo,[echo_max]),axis=0)
            pt_echo=pt_echo+self.mapVals['nPPL_dia'][kk]

        # self.mapVals['data_time'] =[time_vector, data_full]
        time_vector= np.array(time_vector)
        self.mapVals['time_vector']=time_vector
        ...
        result1 = {'widget': 'curve',
                   'xData': time_vector[:,0]*1e-3,
                   'yData': [np.abs(data_full), np.real(data_full), np.imag(data_full)],
                   'xLabel': 'Time (ms)',
                   'yLabel': 'Signal amplitude (mV)',
                   'title': 'Signal vs time',
                   'legend': ['abs data', 'real', 'imag'],
                   'row': 0,
                   'col': 0}
