...
class GRE2D5(blankSeq.MRIBLANKSEQ):
    def __init__(self):
        ...
        self.addParameter(key='sliceAmp', string='选层编码步进 (o.u.)', val=0.003, field='IM')
        self.addParameter(key='nsliPoints', string='选层方向点数 (o.u.)', val=4, field='IM')
    ...
    def sequenceRun(self, plot_seq=0):
        ...
        for i in range(self.nScans):
            for j in range(self.nPoints):
                for k in range(self.nsliPoints):
                    phase_amp = (self.nPoints/2 - j) * self.phaseAmp
                    slice_amp = (self.nsliPoints/2 - k) * self.sliceAmp
                    rf_ex_phase = 0.5*117*((j+k)*(j+k) + (j+k) + 2)%360

                    tim = 1e5 + (i * self.nPoints *self.nsliPoints + j*self.nsliPoints) * self.t_r
        ...
        self.endSequence(self.nsliPoints * self.nPoints * self.nScans * self.t_r + 2e6)
        # --------------------- ↑序列结束↑ ---------------------
    ...
    def sequenceAnalysis(self):
        ...
        data_full = self.decimate(data_average, self.nPoints, self.nsliPoints)
        ksp = self.mapVals['ksp'] = np.reshape(data_full, (self.nPoints, self.nPoints, self.nsliPoints))
